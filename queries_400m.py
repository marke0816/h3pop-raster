# TODO: error handling
# index country and hex id to each lat, lng pair
...
query = """
SELECT country, population, h3 FROM hex_pops
...
def get_top_centroids(count, plot=False):
    ...
    return h3raster.h3list_to_centroids(h3_list), country_counts_dict, top_count[['country', 'h3', 'lat', 'lng']]
...
def get_top_centroids_US_EUR_choose_count(US_count, EUR_count, plot=False):
    ...
    - Dictionary with country counts.
    - DataFrame with columns ['country', 'h3', 'lat', 'lng'] of selected hexes.
    """
    ...
    return h3raster.h3list_to_centroids(h3_list), country_counts_dict, combined_df[['country', 'h3', 'lat', 'lng']]
...
def get_top_centroids_by_strategy(total_count, method='population', 
    ...
    - Tuple:
        ...
        3. DataFrame with columns ['country', 'h3', 'lat', 'lng'] of selected hexes.
    """
    ...
    return h3raster.h3list_to_centroids(h3_list), allocation.to_dict(), final_df[['country', 'h3', 'lat', 'lng']]